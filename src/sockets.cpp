...
RawSocket::RawSocket(const std::string& host, int port)
...
void RawSocket::accept() {
    ...
    std::cout << "[metacar] " << host_ << ":" << port_
              << " connected to " << addr_str << ":" << ntohs(client_addr.sin_port) << std::endl;
}
...
void RawSocket::send(const std::string& data) {
    ...
    while (total_sent < HEADER_SIZE) {
        ...
        if (sent <= 0) throw std::runtime_error("Send failed (header)");
        total_sent += sent;
    }
    ...
    while (total_sent < body_size) {
        ...
        if (sent <= 0) throw std::runtime_error("Send failed (body)");
        total_sent += sent;
    }
}
...
ModelSocket::ModelSocket(const std::string& host, int port)
    : raw_socket_(host, port) {}
...
StreamingSocket::StreamingSocket(const std::string& host, int port)
    : raw_socket_(host, port) {}
